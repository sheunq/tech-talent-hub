...
const mapBackendJobToJobCard = (backendJob: BackendStoredJob): Job => {
  ...
  return {
    ...
    descriptionExcerpt: backendJob.mainDescription, // Use main description for both excerpt and full view
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch jobs from the server.');
        
        let allApiJobs: BackendStoredJob[];
        try {
            allApiJobs = await response.json();
        } catch (e) {
            throw new Error("Failed to parse job data from the server.");
        }
      ...
      <Card className="shadow-xl rounded-xl overflow-hidden">
        ...
        <CardFooter className="p-6 sm:p-8 bg-muted/30 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          {job.applyUrl ? (
            ...
            <Button size="lg" className="w-full sm:w-auto text-base py-3" disabled>
